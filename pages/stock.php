    ...
    <br>
        ...
        <div class="container pt-4 ">
            <table class="table table-striped table">
                ...
                <tbody>
                    <?php
                    $sql = 'select s.idstock, pr.nom_prod,pr.caract,s.cantidad,pr.precio,m.nom_marca,em.raz_social  
                    from  stock s 
                    INNER JOIN productos pr ON s.idproducto = pr.idproducto 
                    INNER JOIN marcas m ON pr.idmarca=m.idmarca 
                    INNER JOIN proveedor p ON s.idproveedor=p.idproveedor 
                    INNER JOIN empresa em ON em.RUC=p.RUC WHERE pr.nom_prod LIKE "%%"  ORDER by pr.nom_prod ;';
                    $item = 0;
